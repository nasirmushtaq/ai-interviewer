...
LANGUAGES = {
    "python": {"lang": "python", "version": "3.10.0", "file": "main.py",
               "label": "Python 3.10", "monaco": "python"},
    "java": {"lang": "java", "version": "15.0.2", "file": "Main.java",
             "label": "Java 15", "monaco": "java"},
    "cpp": {"lang": "c++", "version": "10.2.0", "file": "main.cpp",
            "label": "C++ (GCC 10)", "monaco": "cpp"},
}


def list_languages() -> list[dict]:
    return [
        {"id": k, "label": v["label"], "monaco": v["monaco"]}
...
def _resolve_version(lang_cfg: dict) -> str:
    ...
    for rt in installed_runtimes():
        names = [rt.get("language")] + list(rt.get("aliases", []))
...
def _execute_one(lang_cfg: dict, version: str, source: str, stdin: str) -> dict:
    ...
    r = httpx.post(
        f"{settings.PISTON_URL}/api/v2/execute", json=payload, timeout=40
    )
    r.raise_for_status()
    return r.json()
...
def run_tests(language: str, source: str, tests: list[dict]) -> dict:
    ...
    if not lang_cfg:
        return {"error": f"Unsupported language: {language}", "results": [],
                "passed": 0, "total": 0}
    if not tests:
    ...
    for t in tests:
        try:
            ...
            results.append({
                ...
                "status": "Error", "passed": False, "hidden": t.get("hidden", False),
            })
            continue
