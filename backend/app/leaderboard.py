...
def _rank_query(db: Session, track: str | None, since: datetime | None):
    ...
    q = q.group_by(database.InterviewReport.user_id).order_by(
        func.max(database.InterviewReport.overall_score).desc()
    )
    return q
...
def leaderboard(
    db: Session, track: str | None = None, limit: int = 50, me_id: int | None = None
) -> dict:
